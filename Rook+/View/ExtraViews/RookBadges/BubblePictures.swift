...
public class BubblePictures: NSObject {
    ...
    public init(collectionView: UICollectionView, configFiles: [BPCellConfigFile], layoutConfigurator: BPLayoutConfigurator = BPLayoutConfigurator(), width: CGFloat) {
    ...
    private func assignAlignment() {
        ...
        let emptyWidthSpace = (self.width ?? 100) - bubblesTotalWidth - 0.01
        let emptyWidthSpaceDivided = (((self.width ?? 100) - bubblesTotalWidth) / 2.0) - 0.01
    ...
    fileprivate var maxNumberOfBubbles: Int {
        let width: CGFloat = self.width ?? 100
        let height = self.collectionView?.bounds.height ?? 0.0
